...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Build data payload based on selected role requirements
      const payload = {
        name: form.name,
        email: form.email,
      ...
      const endpoint = role === 'SHIPOWNER' ? '/api/auth/register/shipowner' : '/api/auth/register/inspector';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 sm:px-6 lg:px-8 py-8">
      <div className="max-w-md w-full space-y-6 bg-white p-8 rounded-xl shadow-md border border-gray-200">
        ...
        {error && (
          <div className="bg-red-50 border border-red-400 text-red-700 text-sm p-3 rounded-lg text-center font-medium">{error}</div>
        )}

        <form className="space-y-4" onSubmit={handleSubmit}>
          {/* Select Choice dropdown option */}
          <div>
          ...
          {/* Render extra input forms fields ONLY if user selects SHIPOWNER */}
          {role === 'SHIPOWNER' && (
          ...
          <div>
            <button
              type="submit"
              disabled={loading}
              className="w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-lg text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-blue-400 transition-colors shadow-sm"
            >
